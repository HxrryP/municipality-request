        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <form action="{{ route('requests.index') }}" method="GET">
                    <div class="space-y-4">
                        <div class="flex flex-col md:flex-row md:items-end md:space-x-4 space-y-4 md:space-y-0">
                            <div class="flex-grow">
                                <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Search Requests</label>
                                <div class="relative rounded-md shadow-sm">
                                    <input type="text" name="search" id="search" class="focus:ring-blue-500 focus:border-blue-500 block w-full pl-4 pr-10 py-2 sm:text-sm border-gray-300 rounded-md" placeholder="Search by tracking number or service" value="{{ request('search') }}">
                        ...
                        <div class="flex flex-col md:flex-row md:items-end space-y-4 md:space-y-0 md:space-x-4">
                            ...
                            <div class="flex space-x-2">
                                ...
                                @if(request('search') || request('status') || request('service') || request('date_from') || request('date_to'))
                                    <a href="{{ route('requests.index') }}" class="inline-flex justify-center py-2 px-4 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                        Clear Filters
                                    </a>
                                @endif
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h3 class="text-lg font-medium text-gray-900">{{ $requests->where('status', 'pending')->count() + $requests->where('status', 'processing')->count() + $requests->where('status', 'payment_required')->count() }}</h3>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h3 class="text-lg font-medium text-gray-900">{{ $requests->where('status', 'completed')->count() }}</h3>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <h3 class="text-lg font-medium text-gray-900">{{ $requests->count() }}</h3>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 border-b border-gray-200">
                    <h3 class="text-lg font-medium text-gray-900 mb-6">Requests</h3>
                    
                    @if($requests->count() > 0)
                        <div class="space-y-6">
                            @foreach($requests as $request)
                                <div class="border rounded-lg overflow-hidden hover:shadow-md transition">
                                    <div class="px-6 py-4 flex flex-col md:flex-row md:justify-between md:items-center gap-4">
                                        <div class="flex-grow">
                                            <div class="flex items-start gap-4">
                                                <div class="hidden md:flex h-12 w-12 rounded-full bg-blue-100 items-center justify-center flex-shrink-0">
                                                    @switch(substr($request->service->category->slug, 0, 4))
                                                        @case('busi')
                                                            <svg class="h-6 w-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                <div>
                                                    <h4 class="text-lg font-medium text-gray-900">{{ $request->service->name }}</h4>
                                        ...
                                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                                            ...
                                            <div class="flex space-x-2">
                                                <a href="{{ route('requests.show', $request) }}" class="inline-flex items-center px-3 py-1 border border-transparent rounded text-xs font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
                                                ...
                                                <a href="{{ route('requests.track', $request) }}" class="inline-flex items-center px-3 py-1 border border-gray-300 rounded text-xs font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none">
                                                    <svg class="h-3 w-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                @if($request->status == 'payment_required')
                                                    <a href="{{ route('payments.show', $request) }}" class="inline-flex items-center px-3 py-1 border border-transparent rounded text-xs font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none">
                                                        <svg class="h-3 w-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Get started by creating a new service request.</p>
                            <div class="mt-6">
            ...
            <div class="mt-6 text-center">
                <a href="{{ route('services.index') }}" class="inline-flex items-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    <svg class="h-5 w-5 mr-2 -ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                    </svg>
                    Request a New Service
